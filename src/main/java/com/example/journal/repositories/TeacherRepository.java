...
public interface TeacherRepository extends JpaRepository<Teacher, Long>{
	...
	@Query("SELECT DISTINCT st FROM Student st, Subject su, Teacher tc, CTSMtM c "
			+ "WHERE tc.id = ?1 "
			+ "AND su.id = ?2 "
	...
	@Query("SELECT mk FROM Mark mk "
			+ "WHERE mk.studentId = ?1 "
			+ "AND mk.subjectId = ?2")
	List<Mark> findStudentMarksBySubject(Long studentId, Long subjectId);
